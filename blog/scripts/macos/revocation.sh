...
# The command-line surface for it is narrower than the Linux one, which is the
# thing a reader following a Linux instruction runs into.

# Fetch the certificate this site serves, so the checks below have something real
/usr/bin/openssl s_client -connect rlwilliamson.dev:443 -servername rlwilliamson.dev </dev/null 2>/dev/null | /usr/bin/openssl x509 -outform DER -out /tmp/leaf.der && /usr/bin/openssl x509 -inform DER -in /tmp/leaf.der -noout -subject
...
/usr/bin/openssl x509 -inform DER -in /tmp/leaf.der -outform PEM -out /tmp/leaf.pem && security verify-cert -c /tmp/leaf.pem -p ssl 2>&1 | head -4
