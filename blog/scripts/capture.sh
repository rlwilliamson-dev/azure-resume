...
#   ./capture.sh alma --block 2 -- 'pvcreate /dev/loop0 /dev/loop1; pvs'
#
...
if [[ $blocks -gt 0 ]]; then
  ...
  # its own. The VM already uses low-numbered loop devices for podman storage,
  # so never assume /dev/loop0: provision, then hand the real paths to the
  # command as $DEV0, $DEV1, ... and $DEVS.
  # Device-mapper state lives in the VM's kernel, not in the container, so a
  # volume group left behind stays visible to the next run through
  # /dev/mapper/control. Reset first, using the backing-file name to find our
  # own devices and nothing else. The VM has lvm and mdadm natively.
  # Removing the loop device does not remove the device-mapper node built on
  # top of it, and an orphaned node stays visible to the next container through
  # /dev/mapper/control. That is what makes a second capture report "volume
  # group already exists" against devices that were just wiped.
  #
  # dmsetup remove_all is safe here specifically because this VM is disposable
  # and uses plain partitions for its own root; the only device-mapper nodes on
  # it are the ones these captures create.
  reset_devices='
  ...
  # The VM already uses low-numbered loop devices for podman storage, so never
  # assume /dev/loop0: provision, then hand the real paths to the command as
  # $DEV0, $DEV1, ... and $DEVS.
  devs="$(
    podman machine ssh "sudo sh -c '
      for i in \$(seq 0 $((blocks - 1))); do
        truncate -s 512M /var/tmp/capture-\$i.img
        losetup -f --show /var/tmp/capture-\$i.img
      done'" 2>/dev/null | tr -d '\r' | tr '\n' ' '
  )"
  devs="${devs% }"
  [[ -n $devs ]] || die "loop device provisioning returned nothing"

  # Expose the device paths by position; never assume /dev/loop0.
  prelude="DEVS=\"$devs\""$'\n'
  i=0
  for d in $devs; do
    prelude+="DEV$i=$d"$'\n'
