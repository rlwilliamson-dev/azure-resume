...
| **Custom subpages**: `/now`, `/uses`, `/whoami`, `/hall-of-fame` | A resume is one snapshot of one person; these pages add depth and personality |
...
| **A+ security headers** (COOP, COEP, CORP, CSP, HSTS, X-Frame-Options) | The base challenge doesn't touch security headers; I wanted [an A on securityheaders.com](https://securityheaders.com/?q=rlwilliamson.dev) |
| **`security.txt` (RFC 9116)** | Standard practice for any publicly-facing site to declare a security contact |
| **`robots.txt` + `sitemap.xml`** | Proper SEO hygiene; sitemap auto-generated by Astro for the blog |
...
| **Custom 404 page** with `staticwebapp.config.json` override returning real 404 status | SWA's default is a 200 with `/index.html`, which search engines see as duplicate content |
| **Inline animated architecture diagram** showing live data flow browser → Cosmos | Helps recruiters who skim by giving them something visual to grasp |
| **Boot animation + a few easter eggs** | Discoverable surprises for engineers who poke at the site. I won't spoil what's in there |
...
| Layer | Tech |
|---|---|
| Static resume | Single-file HTML with inline CSS/JS, `counter.js`, `staticwebapp.config.json`, `me.jpg`, SVG favicon |
| Blog | Astro 6.3 with Content Layer API, Tokyo Night Shiki, `@astrojs/rss`, `@astrojs/sitemap` |
| Custom pages | `/now`, `/uses`, `/whoami`, custom `/404.html` |
...
| Custom domain | Namecheap DNS → Azure SWA, DigiCert-issued SSL, CAA record |
| Commit signing | SSH signing with a dedicated `id_ed25519_signing` key |
| Cost | $0/month + $12/year for the domain |
...
│   ├── 404.html                              # custom 404 (terminal-themed)
...
2. **`build_and_deploy_job`** — `needs: test`; checks out, sets up Node 22, builds the Astro blog into `frontend/blog/`, then deploys the whole `frontend/` directory plus the Python Function to Azure Static Web Apps (preview environment on PRs, production on `main`)
